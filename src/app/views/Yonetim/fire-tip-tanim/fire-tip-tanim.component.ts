...
import { ConnFireTipTanimModel, SabitservService } from 'src/app/services/SabitSrc';
import { CofirmsrcService } from 'src/app/utils/confirm-dialog/cofirmsrc.service';

@Component({
  selector: 'app-fire-tip-tanim',
  templateUrl: './fire-tip-tanim.component.html',
  styleUrls: ['./fire-tip-tanim.component.scss'],
...
export class FireTipTanimComponent implements OnInit {
  @BlockUI() blockUI!: NgBlockUI;
  @ViewChild('gridFireTipTanim', { static: false }) grid!: DxDataGridComponent;
  @Input() data:any;     
  yetki:KullaniciYetki;
  datalist: ConnFireTipTanimModel[]=[];    
  silgoster:boolean=false;  
  secilidata:ConnFireTipTanimModel; 
  ...
  constructor(  
    ...
    {  
      this.secilidata=new  ConnFireTipTanimModel();  
  ...
  async kaydet(){   
      ...
      var sonuc = await this.sabitsrc.FireTipTanimEkle(this.secilidata,this.secilidata.Id>0?IslemTipi.Guncelle:IslemTipi.Ekle);
  ...
  async kayitsil(){    
    ...
    .then(async (confirmed:any) => 
    {
      if(confirmed.sonuc==true)  {
        ...
        var sonuc = await this.sabitsrc.FireTipTanimEkle(this.secilidata,IslemTipi.Sil);
          if(sonuc.Success==true){
