...
import { IslemTipi, EkranMesaj, ConnKulDepartmanModel, GenelApi } from 'src/app/services/GenelSrc';
...
@Component({
  selector: 'app-kullanici-departman',
  templateUrl: './kullanici-departman.component.html',
  styleUrls: ['./kullanici-departman.component.scss'],
...
export class KullaniciDepartmanComponent implements OnInit {
  @BlockUI() blockUI!: NgBlockUI;
  @ViewChild('gridKullDepartman', { static: false }) grid!: DxDataGridComponent;
  @Input() data:any;     
  yetki:KullaniciYetki;
  datalist: ConnKulDepartmanModel[]=[];    
  silgoster:boolean=false;  
  secilidata:ConnKulDepartmanModel; 
  ...
  constructor(  
    ...
    {  
      this.secilidata=new  ConnKulDepartmanModel();  
  ...
  async kaydet(){   
      ...
      var sonuc = await this.sabitsrc.KullDepartmanEkle(this.secilidata,this.secilidata.Id>0?IslemTipi.Guncelle:IslemTipi.Ekle);
  ...
  async DataListele()  {
    this.blockUI.start(EkranMesaj.Listele);
      (await this.sabitsrc.GetKullDepartman(0)).subscribe(
        data=>{
          this.blockUI.stop(); 
          if(!data.Success){
  ...
  async kayitsil(){    
    ...
    .then(async (confirmed:any) => 
    {
      if(confirmed.sonuc==true)  {
        ...
        var sonuc = await this.sabitsrc.KullDepartmanEkle(this.secilidata,IslemTipi.Sil);
          if(sonuc.Success==true){
