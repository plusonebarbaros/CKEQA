...
import { DtsOnayKategoriModel, DtsOnayTanimModel, OnaySurevSrcService } from 'src/app/services/OnaySrc';
...
import { OnayTanimDetayComponent } from '../onay-tanim-detay/onay-tanim-detay.component';

@Component({
  selector: 'app-onay-tanim',
  templateUrl: './onay-tanim.component.html',
  styleUrls: ['./onay-tanim.component.scss'],
...
export class OnayTanimComponent implements OnInit {
  ...
  constructor( 
    ...
    {  
      ...
      this.yetki = this.kulsrc.userperm.filter((x)=>x.YetkiKodu=="YT0017")[0];
    } 
  ...
  async kaydet(){   
      if(this.secilidata.KategoriId<=0){
        this.alertify.warning("Ana Kategori Alanı Zorunludur!") 
        return;
      }
      if(this.secilidata.Tanim=="" || this.secilidata.Tanim==null){
        this.alertify.warning("Tanım Alanı Zorunludur!") 
        return;
      }  

      this.blockUI.start("Kayıt Başladı...");
      var sonuc = await this.onaysrc.OnayTanimEkle(this.secilidata,this.secilidata.Id > 0 ? IslemTipi.Guncelle : IslemTipi.Ekle);
  ...
  async DataListele()  {
    this.blockUI.start(EkranMesaj.Listele);
      (await this.onaysrc.GetOnayTanim(0,"","","")).subscribe(
        data=>{ 
          this.blockUI.stop(); 
  ...
  yeniEkle(content:any){  
    ...
    this.secilidata.OnaySiralimi=true;
    this.secilidata.OtomatikOnaySure=15;
    this.secilidata.GerekliOnay=1;
    this.secilidata.GerekliRet=0;
  ...
  onaydetay(content:any,data:any){
  ...
  async dataSil(){    
      ...
      .then(async (confirmed:any) => 
      {
        if(confirmed.sonuc==true)  {
          ...
          var sonuc = await this.onaysrc.OnayTanimEkle(this.secilidata,IslemTipi.Sil);
            if(sonuc.Success==true){
  ...
  kayitdetay(data:DtsOnayTanimModel){ 
    this.tabService.addTab(new Tab(OnayTanimDetayComponent, data.Tanim , { parent: "AppComponent", data:data,yetki:this.yetki }));
  }
}
