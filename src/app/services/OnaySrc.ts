...
import { ReturnValues, Result, IslemTipi } from './GenelSrc';
import { KullaniciSrcService, KullaniciModel } from './KullaniciSrc';
import { ConnTeklifKalem, ConnTeklifFirma, ConnTeklifMas } from './SatinAlmaSrc';
...
export class OnaySurevSrcService { 
  ...
  constructor( 
    ...
    async OnayYonlendir(list:DtsOnaySurecMasterModel[],kull:DtsOnayTanimDetayModel,YonlendirGeriGelsin:boolean,Aciklama:string):Promise<ReturnValues>  {
      const headers = new HttpHeaders(
        {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
        ...
        const body =  JSON.stringify({ 
          "List":  list,
          "Kullanici":kull, 
          "YonlendirGeriGelsin":YonlendirGeriGelsin, 
          "Token":this.kullsrc.token,
          "Aciklama":Aciklama
        }); 
...
{ 
  Talep:ConnTeklifKalem[]=[]; 
  Teklif!:TeklifOnay; 
}

export class TeklifOnay
{ 
  Firma:ConnTeklifFirma[]=[];
  Kalem:ConnTeklifKalem[]=[]; 
  Baslik!:ConnTeklifMas;  
}